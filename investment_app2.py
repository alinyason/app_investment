...
<style>
    ...
    .project-card { #стиль карточек проекта
        background: #f8f9ff; #синий фон
        border-left: 4px solid #2962ff;
        border-radius: 4px;
        padding: 16px 20px;
        margin-bottom: 16px;
    }
    .advice-box { #стиль для советов и предупреждений
        background: #fff8e1; #жёлтый фон
    ...
    .advice-box.good { #совет хороший
        background: #e8f5e9;
        border-left-color: #2e7d32;
    }
    .criterion-row { #стиль для критериев оценки
        display: flex;
        align-items: center;
        gap: 10px;
        padding: 6px 0;
        ...
        font-size: 0.93rem;
    } #далее бейджи (подходит критерий или нет)
    .badge-ok  { background:#e8f5e9; color:#2e7d32; padding:2px 10px; border-radius:12px; font-weight:600; }
    .badge-bad { background:#ffebee; color:#c62828; padding:2px 10px; border-radius:12px; font-weight:600; }
    .formula-box { #для блока с формулами
        background: #f5f5f5;
        border-radius: 6px;
        padding: 12px 16px;
    ...
    .section-header { #стиль для заголовкой секций
        font-family: 'IBM Plex Mono', monospace;
        font-size: 1rem;
        font-weight: 600;
        color: #2962ff;
...
# РАСЧЁТНЫЕ ФУНКЦИИ
# ═════════════════════════════════════════════
#сделать таблицу чтобы она выводилась как в лаб рабе
def build_cf_table(p: dict) -> pd.DataFrame:
    """
    Строит детальную таблицу денежных потоков по годам.
    Выручка, переменные и постоянные издержки, прибыль, налог, CF.
    """
    ...
    for t in range(1, p["years"] + 1): #цикл по годам
        ...
        rows.append({
            ...
            "Денежный поток CF": round(cash_flow, 2), #СДП
            "Дисконтированный CF": round(discounted_cf, 2) #ДДП
        })
...
def build_cash_flows(p: dict) -> list: #денежный поток по годам таблица
    ...
    return df["Денежный поток CF"].tolist() #извлекает данную колонку и преобразует в список (это Денежный поток CF)
...
def bep_advice(p: dict, bep: float, npv: float) -> tuple: #рекоммендации
    ...
    if npv >= 0 and bep <= volume: #случай 1 всё отлично
        ...
        return (
            f"✅ Проект эффективен. Точка безубыточности ({bep:,.0f} ед.) "
            f"ниже планового выпуска ({volume:,.0f} ед.). "
    ...
    if bep == float("inf") or margin <= 0: #случай 2 цена ниже переменных затрат
        advices.append(
            f"⚠️ Цена ({p['price']:.2f} Р) ≤ переменных издержек на единицу "
    ...
    if bep > volume: #случай 3  низкий плановый выпуск
        ...
        advices.append(
            f"❌ Точка безубыточности для объёма продукции (BEP) ({bep:,.0f} ед.) превышает плановый выпуск ({volume:,.0f} ед.). "
            f"Проект не достигает безубыточности. Варианты исправления:"
            f"<br>• Увеличить объём выпуска до ≥ {req_volume:,} ед."
            ...
            f"<br>• Снизить пост. издержки до ≤ {max_fixed:,.0f} Р/год (сейчас {p['fixed_cost']:,.0f} Р)"
        )

    if npv < 0 and bep <= volume: #случай 4 NPV отрицательный
        advices.append(
            f"⚠️ Точка безубыточности для объёма продукции (BEP) достигается, но NPV = {npv:,.0f} Р (отрицательный). "
...
with tab1:
    ...
    with st.expander("➕ Добавить новый проект", expanded=len(st.session_state.projects) == 0):
        ...
        with col_l:
            ...
            years      = st.slider("Срок проекта (лет)", 1, 10, 5)
    ...
    if st.session_state.projects:
        ...
        for idx, p in enumerate(st.session_state.projects):
            ...
            c1.metric("Инвестиции",  f"{p['invest']:,.0f} Р")
            c2.metric("Срок",         f"{p['years']} лет")
            c3.metric("Выпуск/год",   f"{p['volume']:,.0f} ед.")
            c4.metric("Цена",         f"{p['price']:,.0f} Р")
            c5.metric("Ставка",       f"{p['rate']}%")

            ci, cd = st.columns([5, 1])
            ci.caption(
                f"Перем. изд.: {p['var_cost']} Р/ед. · "
                f"Пост. изд.: {p['fixed_cost']:,.0f} Р/год · "
                f"Налог: {p['tax']}% · Инфляция: {p['inflation']}%"
            )
            if cd.button("🗑️", key=f"del_{idx}", help="Удалить проект"):
...
with tab2:
    ...
    for r in results:
        ...
        bep_s = f"{r['bep']:,.0f}"  if r["bep"] != float("inf") else "∞"
    ...
    def style_pi(v):
        return "color:#2e7d32;font-weight:600" if v >= 1 else "color:#c62828;font-weight:600"
    ...
    fig_npv = go.Figure(go.Bar(
        ...
        marker_color=["#2e7d32" if r["npv"] >= 0 else "#c62828" for r in results],
        text=[f"{r['npv']:,.0f} Р" for r in results],
    ...
    for i, r in enumerate(results):
        ...
        with st.expander(f"📁 {r['name']}", expanded=(i == 0)):
            ...
            npv_terms = " + ".join(
                f"{cf:,.0f}/(1+{r_dec:.2f})^{t}"
                for t, cf in enumerate(r["cfs"], start=1)
            )

            st.markdown(f"""
<div class="formula-box">
<b>NPV</b> = −I₀ + Σ CF_t / (1+r)^t<br>
NPV = −{p['invest']:,.0f} + {npv_terms}<br>
<b>NPV = {r['npv']:,.0f} Р</b><br><br>
<b>IRR</b> — ставка при которой NPV = 0 &nbsp;→&nbsp; <b>IRR = {irr_str}</b><br><br>
<b>PI</b> = PV(CF) / I₀ = (NPV + I₀) / I₀
&nbsp;= ({r['npv']:,.0f} + {p['invest']:,.0f}) / {p['invest']:,.0f}
&nbsp;= <b>{r['pi']:.3f}</b><br><br>
<b>BEP</b> = Пост.изд. / (Цена − Перем.изд./ед.)
&nbsp;= {p['fixed_cost']:,.0f} / ({p['price']:.2f} − {p['var_cost']:.2f})
&nbsp;= <b>{r['bep']:,.1f} ед.</b>
</div>""", unsafe_allow_html=True)
            ...
            criteria = [
                ...
                (f"BEP ≤ выпуска ({p['volume']:,.0f} ед.)",
                 f"BEP = {r['bep']:,.0f} ед." if r["bep"] != float("inf") else "BEP = ∞",
            ...
            # — Таблица CF по годам —
            st.markdown("**Таблица свободных денежных потоков по годам**") #свободные CF
            df_cf_disp = r["df_cf"].copy()
            for col in df_cf_disp.columns:
                if col != "Год":
                    df_cf_disp[col] = df_cf_disp[col].apply(lambda x: f"{x:,.2f}")
            ...
            fig_cf = go.Figure(go.Bar(
                ...
                y=cfs_vals,
                marker_color=[
                    COLORS[i % len(COLORS)] if v >= 0 else "#ef9a9a"
                    for v in cfs_vals
                ],
                text=[f"{v:,.0f}" for v in cfs_vals],
                textposition="outside",
            ))
            fig_cf.update_layout(
                title="свободные денежные потоки по годам", #Чистая прибыль (CF) по годам
                height=280, margin=dict(t=40, b=10),
                plot_bgcolor="white",
                yaxis=dict(title="CF, Р", gridcolor="#eee"),
                showlegend=False,
            )
            st.plotly_chart(fig_cf, use_container_width=True)

            # — График накопленного дисконтированного CF —
            rate_dec = p["rate"] / 100
            cum = [-p["invest"]]
            for t, cf in enumerate(r["cfs"], start=1):
            ...
            fig_cum.update_layout(
                title=" ДДП или Накопленный дисконтированный CF (окупаемость)",
            ...
            if r["bep"] != float("inf"):
                ...
                fig_bep.add_vline(
                    x=r["bep"], line_dash="dot", line_color="#ff8f00",
                    annotation_text=f"BEP={r['bep']:,.0f} ед.",
                    annotation_position="top right"
                )
                fig_bep.add_vline(
                    x=p["volume"], line_dash="dot", line_color="#2e7d32",
                    annotation_text=f"Выпуск={p['volume']:,.0f} ед.",
...
Это задача **целочисленного линейного программирования (ILP)** — частный случай задачи о рюкзаке.
""")
    ...
    # Инициализация списка групп в session_state
    if "mutex_groups" not in st.session_state:
        st.session_state.mutex_groups = []  # список списков имён проектов
    ...
    if st.session_state.mutex_groups:
        ...
        for g_idx, group in enumerate(st.session_state.mutex_groups):
            ...
            if col_del.button("🗑️", key=f"del_mutex_{g_idx}", help="Удалить группу"):
    ...
    run_lp = st.button("⚙️ Решить задачу оптимизации", type="primary")
    ...
    if run_lp:
        ...
        if status_str == "Optimal":
            ...
            m2.metric("Суммарный NPV",  f"{total_npv:,.0f} Р",
                      delta="максимально возможный")
            m3.metric("Использовано бюджета", f"{total_inv:,.0f} Р")
            ...
            with col_g1:
                ...
                fig_port = go.Figure(go.Bar(
                    x=names_all,
                    y=npv_all,
                    marker_color=["#2962ff" if inc else "#bdbdbd" for inc in in_port],
                    text=[
                        f"{npv:,.0f} Р<br>{'✅ В портфеле' if inc else '—'}"
            ...
            fig_budget.add_trace(go.Bar(
                ...
                marker_color="#e0e0e0",
                text=f"{max(total_budget - total_inv, 0):,.0f} Р",
                textposition="inside",
            ))
            ...
            budget_terms = " + ".join(
                f"{r['invest']:,.0f}·x_{i+1}"
                for i, r in enumerate(candidates)
            )
            lp_lines.append(f"{budget_terms} ≤ {total_budget:,.0f}  (бюджет)")
            if min_projects > 0:
                lp_lines.append(
            ...
            lp_lines.append(f"")
            lp_lines.append(f"**Оптимальное значение Z = {total_npv:,.0f} Р**")
            ...
            advice_lp = (
                ...
                f"максимальный суммарный NPV = {total_npv:,.0f} Р "
                f"при бюджете {total_budget:,.0f} Р. "
                f"Загрузка бюджета: {budget_use:.1f}%."
            ) if selected else (
                "⚠️ При заданных ограничениях ни один проект не может быть включён в портфель. "
...
with tab4:
    ...
    st.subheader("⚠️ Анализ риска и чувствительности")
    st.markdown(
    ...
    fig_sens.add_hline(y=sel_r["npv"], line_dash="dot", line_color="#2962ff", line_width=1,
                       annotation_text=f"Базовый NPV={sel_r['npv']:,.0f}",
    ...
    for label, key in sens_params.items():
        ...
        elast_rows.append({
            "Параметр":           label,
            "Базовое значение":   f"{sel_p[key]:,.2f}",
    ...
    for label, npv_lo, npv_hi, _ in tornado_rows:
        ...
        fig_torn.add_trace(go.Bar(
            ...
            marker_color="#ef9a9a",
            showlegend=False,
            hovertemplate=f"{label}<br>−20%: {npv_lo:,.0f} Р<extra></extra>",
        ))
        fig_torn.add_trace(go.Bar(
            name=label,
            y=[label],
            ...
            marker_color="#90caf9",
            showlegend=False,
            hovertemplate=f"{label}<br>+20%: {npv_hi:,.0f} Р<extra></extra>",
        ))

    fig_torn.add_vline(x=base_npv, line_dash="dash", line_color="#2962ff", line_width=2,
    ...
    with st.expander("⚙️ Настроить коэффициенты сценариев", expanded=False):
    ...
    sc_colors = ["#ef5350", "#42a5f5", "#66bb6a"]

    fig_sc = go.Figure(go.Bar(
        x=sc_names,
        y=sc_npvs,
        marker_color=sc_colors,
        text=[f"{v:,.0f} Р" for v in sc_npvs],
        textposition="outside",
        width=0.45,
    ))
    ...
    run_mc = st.button("▶️ Запустить симуляцию Монте-Карло", type="primary")
    ...
    if run_mc:
        ...
        mc1.metric("Среднее NPV",    f"{mean_npv:,.0f} Р")
        mc2.metric("Ст. отклонение", f"{std_npv:,.0f} Р")
        mc3.metric("P(NPV < 0)",     f"{prob_loss:.1f}%",
                   delta="риск убытка", delta_color="inverse")
        mc4.metric("90% доверит. интервал",
                   f"[{npv_5pct:,.0f} ; {npv_95pct:,.0f}]")
        ...
        bin_colors  = ["#ef5350" if c < 0 else "#66bb6a" for c in bin_centers]
        ...
        fig_mc.add_trace(go.Bar(
            ...
            width=bin_w * 0.9,
            name="Частота",
            hovertemplate="NPV ≈ %{x:,.0f} Р<br>Частота: %{y}<extra></extra>",
        ))
        fig_mc.add_vline(x=0, line_dash="dash", line_color="black",  line_width=2,
                         annotation_text="NPV=0", annotation_position="top right")
        fig_mc.add_vline(x=mean_npv, line_dash="dot", line_color="#2962ff", line_width=2,
                         annotation_text=f"Среднее={mean_npv:,.0f}",
                         annotation_position="top left")
        fig_mc.add_vrect(x0=npv_5pct, x1=npv_95pct,
                         fillcolor="#42a5f5", opacity=0.08,
        ...
        if prob_loss < 10:
            ...
            risk_text  = (
                ...
                f"Среднее NPV ({mean_npv:,.0f} Р) значительно выше нуля."
            )
    ...
    if len(results_r) >= 2:
        ...
        def safe_norm(vals):
            """Нормализует список в диапазон 0..1."""
        ...
        for i, r in enumerate(results_r):
            vals = [
                norm_npv[i], norm_pi[i], norm_bep[i],
                norm_crit[i], norm_dpp[i], norm_npv[i],  # замыкаем фигуру
            ]
